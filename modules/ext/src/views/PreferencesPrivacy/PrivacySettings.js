...
const languageSwitchSelector = createSelector(
...
const notificationBlockerSelector = createSelector(
  s => s.notificationBlockerEnabled,
  (...args) => args,
)

const cookieMonsterSelector = createSelector(
...
export const CookieMonsterSetting = ({ setReloadTrue }) => {
  const [cookieMonsterEnabled, cookieMonsterOnlyThirdParty] = useConnect(
    cookieMonsterSelector,
  )
  ...
  return (
    <ToggleSettingItem
      title={t('Cookie Monster')}
      subHeading={t('Delete Cookies when tab is closed')}
      toggleState={cookieMonsterEnabled}
      toggleEvent={toggleCookieMonster(cookieMonsterEnabled)}
      postToggleEvent={setReloadTrue}
      linkPath="features/cookie-monster"
      SelectComponent={() => (
        <CookieMonsterSelect
          checked={cookieMonsterEnabled}
          onlyThirdParty={cookieMonsterOnlyThirdParty}
          changeMode={() => {
            toggleCookieMonsterMode(cookieMonsterOnlyThirdParty)
            setReloadTrue()
          }}
        />
      )}
      aria-label={t('Cookie Monster. Delete Cookies when tab is closed')}
    />
  )
}
...
const LanguageSwitchToggle = ({ setReloadTrue, disabled }) => {
  const dispatch = useDispatch()
  const { t } = useTranslation()
  const [languageSwitchEnabled] = useConnect(languageSwitchSelector)
  const toggle = v => () => {
    dispatch(
  ...
  return (
    <ToggleSettingItem
      title={t('Language Warp')}
      subHeading={t(
        'Sets your language and locale settings to match the connected proxy',
      )}
      linkPath="features/languagewarp"
      toggleState={languageSwitchEnabled}
      toggleEvent={toggle(languageSwitchEnabled)}
      postToggleEvent={setReloadTrue}
      disabled={disabled}
      ControlComponent={props => (
        <Flex alignItems="center">
      ...
      aria-label={t(
        'Language Switch. Sets your language and locale settings to match the connected proxy.',
      )}
    />
  )
}
...
const WorkerBlockToggle = ({ setReloadTrue }) => {
  ...
  return (
    <ToggleSettingItem
      ...
      linkPath="features/worker-block" // Does not exist
...
const privacySettingsSelector = createSelector(
  s => s.activeTabId,
  s => s.webRTCEnabled,
...
export default () => {
  ...
  const [
    ...
    currentLocation,
    splitPersonalityEnabled,
  ] = useSelector(privacySettingsSelector)
  ...
  const [notificationBlockerEnabled] = useConnect(notificationBlockerSelector)
  ...
  return (
    <Flex flexDirection="column" innerRef={viewContainerRef}>
      ...
      <Box>
        <SettingGroup>
          <CookieMonsterSetting setReloadTrue={setReloadTrue} />
